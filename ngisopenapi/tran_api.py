...
def get_available_projections(kategori=None, system=None):
    ...
    response = requests.get(
        f"{BASE_URL}/transformering/v1/projeksjoner", params=params)
...
def transform_coord(x, y, fra, til, z=None, t=None):
    ...
    if t:
        params["t"] = t
    response = requests.get(
        f"{BASE_URL}/transformering/v1/transformer", params=params)
    return response.json()
...
def transform_coords(coords, fra, til):
    ...
    response = requests.post(
        f"{BASE_URL}/transformering/v1/transformer?fra={fra}&til={til}", json=data, headers=headers)
    return response.json()
